...
function GetCollecteClient() {
  ...
  useEffect(()=>{
    ...
    axios({
      method:"get",
      url:`http://localhost:8500/collecte/forclient?month=${month}&year=${year}&id_client=${idUser}`,
      responseType: 'json',
      headers: {
          'Authorization': token,
    ...
    return (
        ...
        <div className="pd-ltr-20 xs-pd-20-10">
          ...
          <div className="card-box mb-30">
						<div className="pd-20">
							<h4 className="h4">Collectes de déchets du client</h4>
						</div>
						<div className="pb-20">
            <table className="checkbox-datatable table nowrap">
                ...
                {collecteDechets.map((collecte) => 
                          ...
                          <td>{collecte.nom}<br/> {collecte.postnom}<br/>{collecte.prenom}</td>
                          <td>{collecte.sexe}</td>
                          <td>
  ...
  export default GetCollecteClient;